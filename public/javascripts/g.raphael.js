    ...
    Raphael.fn.g.axis = function (x, y, length, from, to, steps, orientation, labels, type, dashsize) {
        ...
        if (+orientation == 1 || +orientation == 3) {
            ...
            while (Y >= y - length) {
                ...
                text.push(this.text(x + addon, Y, (labels && labels(label, d)) || (Math.round(label) == label ? label : +label.toFixed(rnd))).attr(this.g.txtattr).attr({"text-anchor": orientation - 1 ? "start" : "end"}));
                label += d;
                Y -= dx;
            }
            ...
            while (X <= x + length) {
                ...
                text.push(txt = this.text(X, y + addon, (labels && labels(label, d)) || (Math.round(label) == label ? label : +label.toFixed(rnd))).attr(this.g.txtattr));
                var bb = txt.getBBox();
                if (prev >= bb.x - 5) {
